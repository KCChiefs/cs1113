...
{- TOOK THIS OUT OF CODE TO MAKE IT EASIER ON TESTING (NO MORE HOLE NOTIFICATIONS)
...
-- the specified "emptySet" abstration, representing the empty set
-- with an empty list.
emptySet = mkSet nil
...
combine l1 nil = l1
combine nil l2 = l2
combine (x::y) (x2::y2) = ite (member x (x2::y2))
                          (combine y (x2::y2))
                          (x::(combine y (x2::y2)))
...
difference (h::t) l2 = ite (member h l2)
                       (difference t l2)
                       (h::(difference t l2))

-- Return the set difference, s1 minus s2
--set_difference: (eq a) => (s1: set a) -> (s2: set a) -> set a
set_difference (mkSet l1) (mkSet l2) = mkSet (difference l1 l2)
...
--Takes one element and a list and returns all pairs of that element
...
--Takes two lists and returns list of all pairs
...
subset_el: (eq a) => list a -> list a -> bool
subset_el nil nil = true
subset_el nil (h::t) = true
subset_el (h::t) nil = false
subset_el (h::t1) l = and (member h l) (subset_el t1 l)

same_el: (eq a) => list a -> list a -> bool 
same_el t1 t2 = and (subset_el t1 t2) (subset_el t2 t1)

set_eql (mkSet l1) (mkSet l2) = same_el l1 l2
...
--instance (Serialize a) => Serialize (set a) where toString s = set_toString s
